...
const props = defineProps<{ options: { value: T; label: string; disabled?: boolean; flag?: string }[]; placeholder: string }>()
...
const fi = (o?: { value: T; flag?: string }) => (o?.flag ? `fi fi-${o.flag}` : 'fi fi-xx')
</script>

<template>
  <div ref="root" class="csel" :class="{ open }">
    <button type="button" class="csel-btn" :class="{ empty: !model }" @click="open = !open">
      <span v-if="model" :class="fi(current())"></span>
      <span class="lbl">{{ current()?.label ?? placeholder }}</span>
    ...
    <Transition name="drop">
      <ul v-if="open" class="csel-list" role="listbox">
        <li v-for="o in options" :key="o.value">
          <button type="button" role="option" :aria-selected="o.value === model" :disabled="o.disabled" :class="{ on: o.value === model }" @click="pick(o.value)"><span :class="fi(o)"></span>{{ o.label }}</button>
        </li>
      </ul>
    </Transition>
  </div>
